...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for l in range(0, self.num_layers-1):
            self.params[f'W{l+1}'] = np.random.randn(dimensions[l], dimensions[l+1]) * weight_scale
        ...
        self.params[f'W{self.num_layers}'] = np.random.randn(dimensions[self.num_layers-1], dimensions[self.num_layers]) * weight_scale
        self.params[f'b{self.num_layers}'] = np.zeros(dimensions[self.num_layers])
    ...
    def loss(self, X, y=None):
        ...
        for l in range(1, self.num_layers):
          cache = []
          w[l] = self.params[f'W{l}']
          b[l] = self.params[f'b{l}']
          if self.normalization == 'batchnorm':
            g[l] = self.params[f'gamma{l}']
            bt[l] = self.params[f'beta{l}']
          ...
          if self.normalization == 'batchnorm':
            scores, batch_cache = batchnorm_forward(scores, g[l], bt[l], self.bn_params[l - 1])
        ...
        w[self.num_layers] = self.params[f'W{self.num_layers}']
        b[self.num_layers] = self.params[f'b{self.num_layers}']
        scores, score_cache = affine_forward(scores, w[self.num_layers], b[self.num_layers])
       ...
       ############################################################################
        ...
        loss, dout = softmax_loss(scores, y)
        dout, dw[self.num_layers], db[self.num_layers] = affine_backward(dout, score_cache)
        ...
        for key in dw:
          ...
          grads[f'W{key}'] = dw[key]
          grads[f'b{key}'] = db[key]
